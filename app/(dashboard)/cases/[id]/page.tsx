          ...
          <div>
            ...
            <div className="flex flex-col gap-4 font-mono text-sm">
              <div className="flex justify-between items-center">
                ...
                <span className="text-recovered" suppressHydrationWarning>PASS ({new Date(c.opened_at).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour12: true, hour: '2-digit', minute: '2-digit' })} IST)</span>
              </div>
              <div className="flex justify-between items-center">
        ...
        <div className="sharp-card flex flex-col font-mono text-sm">
          {c.audit_logs?.map((log, index) => (
            <div key={log.id} className={`p-4 border-b border-border transition-colors flex gap-4 ${getActionColor(log.action).replace('text-', 'bg-').replace('-bg', '-bg/30')}`}>
              <div className="w-24 shrink-0 text-text-muted text-xs" suppressHydrationWarning>
                {new Date(log.created_at).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour12: true, hour: '2-digit', minute: '2-digit', second: '2-digit' })}
              </div>
        ...
        <div className="sharp-card overflow-hidden">
          ...
          {c.interventions?.length ? c.interventions.map(int => (
            <div key={int.id} className="grid grid-cols-4 p-4 border-b border-border last:border-0 font-mono text-sm items-center">
              ...
              <div suppressHydrationWarning>{new Date(int.scheduled_for).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true })}</div>
              <div suppressHydrationWarning>{int.executed_at ? new Date(int.executed_at).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true }) : "—"}</div>
              <div>

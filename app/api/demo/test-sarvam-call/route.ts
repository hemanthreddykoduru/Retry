...
export async function POST() {
  ...
  const operatorNumber = process.env.SARVAM_AGENT_PHONE_NUMBER;
  if (!operatorNumber) {
    return NextResponse.json({ error: 'SARVAM_AGENT_PHONE_NUMBER not configured.' }, { status: 400 });
  }
  ...
  try {
    const result = await triggerSarvamOutboundCall({
      ...
      customer_phone: operatorNumber // Safely only calling the configured operator number
    });
